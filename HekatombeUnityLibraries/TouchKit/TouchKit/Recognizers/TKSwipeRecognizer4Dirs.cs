...
[System.Flags]
public enum TKSwipeDirection4Dirs
{
    Left = (1 << 0),
    Right = (1 << 1),
    Up = (1 << 2),
...
public class TKSwipeRecognizer4Dirs : TKAbstractGestureRecognizer
{
    /// <summary>
    /// The event that fires when a swipe is recognized.
    /// </summary>
    public event System.Action<TKSwipeRecognizer4Dirs> gestureRecognizedEvent;
    ...
    /// </summary>
    public TKSwipeDirection4Dirs completedSwipeDirection { get; private set; }
    ...
    public TKSwipeRecognizer4Dirs() : this(2f)
    { }

    public TKSwipeRecognizer4Dirs(float minimumDistanceCm)
    {
    ...
    private bool checkForSwipeCompletion(TKTouch touch)
    {
		...
		if (swipeAngle >= 225f && swipeAngle <= 315f)
            completedSwipeDirection = TKSwipeDirection4Dirs.Up;
        else if (swipeAngle >= 135f && swipeAngle <= 225f)
            completedSwipeDirection = TKSwipeDirection4Dirs.Left;
        else if (swipeAngle >= 45f && swipeAngle <= 135f)
            completedSwipeDirection = TKSwipeDirection4Dirs.Down;
        else // swipeAngle >= 315f || swipeAngle <= 45f
            completedSwipeDirection = TKSwipeDirection4Dirs.Right;
